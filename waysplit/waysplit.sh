...
pv $MYTMPDIR/osm_as_sxml.scm | {
	...
	| tee \
	>(./store-ways.scm ${WAYDB_OUT}) \
	>(grep '^(way'|./sql-dump.scm|pbzip2 > ${SQL_OUT}) \
	|./fastsxml2xml.scm ;
} 2> $MYTMPDIR/waysplit.log
# serial version
#pv $MYTMPDIR/osm_as_sxml.scm| { ./apply-way-splits.scm write-lines $MYTMPDIR/way-splits.dbm $MYTMPDIR/node-pos.dbm $MYTMPDIR/way-relation.dbm $MYTMPDIR/relation.dbm|tee >(./store-ways.scm ${WAYDB_OUT}) >(grep '^(way'|./sql-dump.scm|pbzip2 > ${SQL_OUT})|./fastsxml2xml.scm ; } 2> $MYTMPDIR/waysplit.log
