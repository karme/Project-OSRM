    ...
    (let ((way-splits (dbm-open <gdbm> :path way-splits :rw-mode :read))
	  ...
	  (failed-restrictions 0)
          (output-result (if (equal? output-format "parallel-pipe")
                           parallel-pipe-out
                           write-lines))
          (lua-luise (let1 p (run-lua-repl :input :pipe :output :pipe)
                       (cute lua-eval-luise (process-output p) (process-input p) <...>))))
      (lua-luise '(funcall dofile "../profiles/bicycle.lua"))
                   ...
                   (let* ((profile-way (lambda(w)
                                         (let1 r (way-add-geometry w
                                                                   (upsample-polyline->4d 'wgs84
                                                                                          ...
                                                                                          50))
                                           (append r
                                                   (append-map (lambda(fwd/bwd l)
                                                                 (assert (string? fwd/bwd))
                                                                 (assert (list? l))
                                                                 (map (lambda(x)
                                                                        `(tag (@ (k ,(string-append "osrm:" fwd/bwd ":" (car x)))
                                                                                 (v ,(x->string (cdr x))))))
                                                                      l))
                                                               '("fwd" "bwd")
                                                               (receive l (lua-luise `(funcall way_info
                                                                                               (funcall fakeway
                                                                                                        ,(way-id r)
                                                                                                        ,(map (lambda(x)
